...
class ImageTextTransformation:
    ...
    def image_to_text(self, img_src, gpt=False):
        # the information to generate paragraph based on the context
        self.ref_image = Image.open(img_src).convert('RGB')
        ...
        if self.args.semantic_segment:
            region_semantic = self.region_semantic_model.region_semantic(img_src, self.args.region_classify_model)
        else:
            region_semantic = " "
        #generated_text = self.gpt_model.paragraph_summary_with_gpt(image_caption, dense_caption, region_semantic, width, height)
        generated_text = "Caption: "+image_caption+"; Dense Caption: " + str(dense_caption)+ "; Region Captions: " + str(region_semantic)
